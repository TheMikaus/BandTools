#!/usr/bin/env python3
"""
Unified sync manager for AudioBrowserOrig.
...
class SyncManager(QObject):
    ...
    Provides a single interface for the widget-based AudioBrowserOrig to interact 
    with different sync backends.
    """
    ...
    def __init__(self, config_dir: Path, parent=None):
        ...
        # Active provider (default to Google Drive for backward compatibility)
    ...
    def getAvailableProviders(self) -> List[Dict[str, Any]]:
    ...
    # Backward compatibility methods for existing code that expects these methods
    
    @property
    def remote_folder_id(self) -> Optional[str]:
        """Get remote folder ID from active provider."""
        return self.active_provider.remote_folder_id
    
    @property
    def service(self):
        """Get service object from active provider (for GDrive compatibility)."""
        if hasattr(self.active_provider, 'service'):
            return self.active_provider.service
        return None
    
    def upload_file(self, local_path: Path, remote_name: Optional[str] = None) -> bool:
        """Upload a file to the active provider."""
        return self.active_provider.upload_file(local_path, remote_name)
    
    def download_file(self, remote_name: str, local_path: Path) -> bool:
        """Download a file from the active provider."""
        return self.active_provider.download_file(remote_name, local_path)
    
    def delete_remote_file(self, remote_name: str) -> bool:
        """Delete a file from the active provider."""
        if hasattr(self.active_provider, 'delete_remote_file'):
            return self.active_provider.delete_remote_file(remote_name)
        logger.warning(f"delete_remote_file not supported by {self.active_provider_name}")
        return False
    
    def list_remote_files(self) -> List[Dict[str, Any]]:
        """List files in the remote folder."""
        return self.active_provider.list_remote_files()
    
    def get_remote_version(self):
        """Get version information from remote (if supported)."""
        if hasattr(self.active_provider, 'get_remote_version'):
            return self.active_provider.get_remote_version()
        return None
    
    def update_remote_version(self, version):
        """Update version file on remote (if supported)."""
        if hasattr(self.active_provider, 'update_remote_version'):
            return self.active_provider.update_remote_version(version)
        return False
    
    def delete_remote_folder(self) -> bool:
        """Delete the entire remote folder (if supported)."""
        if hasattr(self.active_provider, 'delete_remote_folder'):
            return self.active_provider.delete_remote_folder()
        logger.warning(f"delete_remote_folder not supported by {self.active_provider_name}")
        return False
