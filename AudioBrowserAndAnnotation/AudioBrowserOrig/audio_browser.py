...
def compare_fingerprints(fp1: List[float], fp2: List[float], debug: bool = False) -> float:
    ...
    if not fp1 or not fp2:
        if debug:
            log_print(f"  [FP Compare] Empty fingerprint: fp1 length={len(fp1) if fp1 else 0}, fp2 length={len(fp2) if fp2 else 0}")
        return 0.0
    ...
    if debug:
        log_print(f"  [FP Compare] Lengths: fp1={len1}, fp2={len2}")
    ...
    if HAVE_NUMPY:
        ...
        if debug:
            log_print(f"  [FP Compare] Norms: norm1={norm1:.6f}, norm2={norm2:.6f}, dot_product={dot_product:.6f}")
        
        if norm1 > 0 and norm2 > 0:
            similarity = float(dot_product / (norm1 * norm2))
            if debug:
                log_print(f"  [FP Compare] Similarity: {similarity:.6f}")
            return similarity
        else:
            if debug:
                log_print(f"  [FP Compare] Zero norm detected, returning 0.0")
            return 0.0
    else:
...
def find_best_cross_folder_match(target_fingerprint: List[float], fingerprint_map: Dict[str, List[Dict]], threshold: float, debug: bool = False, exclude_filename: Optional[str] = None) -> Optional[Tuple[str, float, Path, str]]:
    ...
    if debug:
        ...
        log_print(f"[FP Match] Total comparisons: {len(all_comparisons)}")
        log_print(f"[FP Match] Matches above threshold ({threshold:.2%}): {len(best_matches)}")
        ...
        for i, comp in enumerate(sorted_comparisons[:10], 1):
            ...
            log_print(f"  {i}. {comp['filename']} -> '{comp['provided_name']}' from {comp['folder'].name}")
            log_print(f"     Raw score: {comp['raw_score']:.4f}, Weighted: {comp['weighted_score']:.4f}{boost_str}{ref_indicator}")
        ...
        if threshold > 0:
            ...
            if near_threshold:
                ...
                for comp in sorted(near_threshold, key=lambda x: x['weighted_score'], reverse=True)[:5]:
                    log_print(f"  {comp['filename']} -> '{comp['provided_name']}': {comp['weighted_score']:.4f}")
    
    if not best_matches:
        if debug:
            log_print(f"[FP Match] No matches found above threshold {threshold:.2%}")
        return None
    ...
    if debug:
        ...
        log_print(f"  Filename: {best_match[0]}")
        log_print(f"  Provided name: '{best_match[5]}'")
        log_print(f"  Raw score: {best_match[2]:.4f}")
        log_print(f"  Weighted score: {best_match[1]:.4f}")
        log_print(f"  Source folder: {best_match[3]}")
        log_print(f"  Folder count: {best_match[4]}")
        log_print(f"  Is reference: {best_match[6]}")
...
<p><em>Built with PyQt6 and developed with heavy use of AI assistance (ChatGPT/Copilot).</em></p>"""
    ...
    def _show_practice_statistics_dialog(self):
        ...
        if hasattr(self, '_practice_stats_dialog') and self._practice_stats_dialog is not None:
            self._practice_stats_dialog.raise_()
            self._practice_stats_dialog.activateWindow()
            return
        
        # Create dialog
        self._practice_stats_dialog = QDialog(self)
        self._practice_stats_dialog.setWindowTitle(f"{APP_NAME} - Practice Statistics")
        self._practice_stats_dialog.resize(800, 600)
        
        # Make it non-modal so users can continue working while viewing statistics
        self._practice_stats_dialog.setModal(False)
        
        layout = QVBoxLayout(self._practice_stats_dialog)
        
        # Add informational note about when statistics are calculated
        info_label = QLabel("<em><b>Note:</b> Statistics are calculated from practice folders when this dialog is opened or refreshed. "
                           "Data is not cached - each refresh analyzes your practice folders in real-time.</em>")
        ...
        # Store reference to text_edit so we can update it
        self._practice_stats_dialog.text_edit = text_edit
        ...
        refresh_button.setToolTip("Regenerate statistics based on current practice folders")
        refresh_button.clicked.connect(lambda: self._refresh_practice_statistics())
        ...
        # Add close button
        close_button = QPushButton("Close")
        close_button.clicked.connect(self._practice_stats_dialog.accept)
        button_layout.addWidget(close_button)
        
        layout.addLayout(button_layout)
        
        # Clean up reference when dialog is closed
        self._practice_stats_dialog.finished.connect(lambda: setattr(self, '_practice_stats_dialog', None))
        
        # Generate and display initial statistics
        self._refresh_practice_statistics()
        
        # Show dialog non-modally
        self._practice_stats_dialog.show()
    
    def _refresh_practice_statistics(self):
        """Refresh the practice statistics display with current data."""
        if not hasattr(self, '_practice_stats_dialog') or self._practice_stats_dialog is None:
            return
        ...
        self._practice_stats_dialog.text_edit.setHtml(html_content)
